...
const CheckoutPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const pickupAddress =
                pickupPoints[orderData.pickupPoint] || pickupPoints.ershov;
            ...
            if (err.response?.status === 401) {
                ...
                setError(
                    `Ошибка при оформлении заказа: ${err.response?.data?.message || err.message}`,
                );
            }
        } finally {
            setIsSubmitting(false);
