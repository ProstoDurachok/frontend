...
const CartPage = () => {
  const { cart, removeFromCart, updateQuantity, totalPrice: rawTotal, totalItems } = useCart();
  ...
  const discountCalculation = useMemo(() => {
    ...
    cart.forEach((item) => {
      ...
      if (item.isLens || item.category?.includes("Линзы") || item.name?.toLowerCase().includes("линзы")) {
        lensesTotal += sum;
      } else {
        baseTotal += sum;
      }
    ...
    // Скидка по сумме ТЕКУЩЕГО заказа (без линз)
    ...
    const percentDiscountAmount = Math.round(totalWithoutLenses * (discountPercent / 100));

    // Приветственный бонус 500 ₽ (если не использован)
    let welcomeBonus = 0;
  ...
  }, [cart, user, rawTotal, isAuthenticated]);
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.1)] p-6 border border-gray-100 sticky top-24">
                ...
                <div className="space-y-4 mb-6">
                  ...
                  {user?.totalSpent >= 0 && (
                    <div className="flex justify-between text-base text-gray-700">
                      ...
                      <span>{user.totalSpent.toLocaleString("ru-RU")} ₽</span>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between text-xl font-bold text-[#c41c20] pt-4 border-t border-gray-200">
                    <span>К оплате:</span>
                    <span className="text-[#e31e24]">{finalTotal.toLocaleString("ru-RU")} ₽</span>
                  </div>
                </div>
                ...
                >
                  ...
                  {!isAuthenticated && <AlertCircle size={24} className="relative z-10 ml-1" />}
                </button>

                {isAuthenticated && welcomeBonus > 0 && (
                  <p className="text-sm text-green-600 mt-3 text-center">
                    При оформлении будет использован бонус 500 ₽
                  </p>
                )}
                ...
                <div className="mt-6 pt-6 border-t border-gray-200 space-y-3">
                  ...
                  {birthdayDiscountActive && (
                    <div className="flex items-center space-x-3 text-sm text-green-600 font-medium">
                      ...
                      <span>Действует скидка ко дню рождения!</span>
                    </div>
                  )}
                </div>
              </div>
